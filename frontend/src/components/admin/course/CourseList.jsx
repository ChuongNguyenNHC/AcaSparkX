...
import { FaCheckCircle, FaTimesCircle, FaEdit, FaSearch, FaPlus, FaImage, FaChalkboardTeacher, FaEyeSlash, FaEye } from 'react-icons/fa';
...
const CourseList = ({ type }) => {
    ...
    const handleHide = async (id) => {
        if (!window.confirm('Hide this course? It will be invisible to students and teachers.')) return;
        try {
            await courseService.hideCourse(id);
            fetchCourses();
        } catch (error) {
            alert('Failed to hide course');
        }
    };

    const handleUnhide = async (id) => {
        if (!window.confirm('Unhide this course? It will be visible again.')) return;
        try {
            await courseService.unhideCourse(id);
            fetchCourses();
        } catch (error) {
            alert('Failed to unhide course');
        }
    };
    ...
    const openCreate = () => {
        setEditingId(null);
        setFormData({ title: '', description: '', category_id: '', tags: [], teachers: [], thumbnail: null, status: 'draft', remove_thumbnail: false });
        setPreviewImg(null);
        setShowModal(true);
    };
    ...
    const openEdit = (course) => {
        ...
        const currentThumb = course.thumbnail;
        let preview = null;
        if (currentThumb) {
            preview = currentThumb.startsWith('http') ? currentThumb : `http://localhost:8000/storage/${currentThumb}`;
        }
    ...
    const handleRemoveThumbnail = () => {
        ...
        setPreviewImg(null); // Clear preview to show default/nothing
    };
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={course.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                <td style={styles.td}>
                                    <ImageWithFallback
                                        src={course.thumbnail && course.thumbnail.startsWith('http')
                                            ? course.thumbnail
                                            : (course.thumbnail ? `http://localhost:8000/storage/${course.thumbnail}` : null)}
                                        alt={course.title}
                                ...
                                <td style={styles.td}>
                                    <div style={{ display: 'flex', gap: '8px' }}>
                                        ...
                                        {(course.status === 'published' || course.status === 'active') && (
                                            <button onClick={() => handleHide(course.id)} title="Ẩn khóa học" style={styles.iconBtnSecondary}><FaEyeSlash /></button>
                                        )}
                                        {course.status === 'hidden' && (
                                            <button onClick={() => handleUnhide(course.id)} title="Hiện khóa học" style={styles.iconBtnSuccess}><FaEye /></button>
                                        )}
                                    </div>
                                </td>
                            </tr>
...
const StatusBadge = ({ status }) => {
    ...
    if (status === 'published' || status === 'active') {
        ...
        color = '#475569'; bg = '#f1f5f9'; label = 'Đã ẩn';
    }
...
const styles = {
    ...
    iconBtnSecondary: { background: 'none', border: 'none', cursor: 'pointer', color: '#64748b', fontSize: '1.1rem' },
